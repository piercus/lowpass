...
function LowPass(options) {
  ...
  this.movAverageArr = new Array(this.channels, this.movAvLength*this.ratioByte);
...
LowPass.prototype._transform = function (chunk, enc, cb) {
  ...
  for (var i = 0; i < chunk.length/(this.ratioByte*this.channels); i++){
    for(var c = 0; c < this.channels; c++){
      console.log(this.ratioByte*(i*this.channels+c));
      var inMob = Math.floor(chunk['readInt'+this.bitDepth+this.endianness](this.ratioByte*(i*this.channels+c))/this.movAvLength);

      var outMob = this.movAverageArr.shift();

      this.movAverageArr[this.movAvLength-1] = inMob;

      //console.log(this.lastChunk,inMob,outMob, inMob+outMob, 0+inMob-outMob);

      buf['writeInt'+this.bitDepth+this.endianness](this.lastChunk+inMob-outMob, this.ratioByte*(i*this.channels+c));
