...
EnergyDetector.prototype._transform = function (chunk, enc, cb) {
  ...
  if(this.end && (this.counter/this.sampleRate > this.end)){
    return cb();
  }
  ...
  for (var i = 0; i < chunk.length/this.ratioByte/this.channels; i++){
    ...
    this.counter++;
    if(!(this.counter%100)){
      console.log(this.counter,(this.counter/this.sampleRate < this.start));
    }
    if(this.start && (this.counter/this.sampleRate < this.start)){
      break;
    }
